...
// ==============================================================
// PATCH 35 (V5): HỆ THỐNG NHÃN "NEW" HOẠT ĐỘNG TỨC THÌ Ở GIÂY SỐ 0
// ==============================================================
(function() {
    // 1. Khởi tạo danh sách "Đã Xem" ngay giây 0
        ...
        if (!localStorage.getItem('vinhloc_megas_initialized')) {
            if (typeof folderDataCache !== 'undefined' && folderDataCache[ROOT_FOLDER_ID]) {
                folderDataCache[ROOT_FOLDER_ID].forEach(item => {
                    // Ân xá toàn bộ đồ cũ (Bỏ qua những mục đang có chữ temp_ vì nó là đồ vừa mới tạo)
    ...
    // 3. CHỐT CHẶN RENDER: Bám rễ ngay lập tức vào hàm vẽ giao diện
    ...
    // 5. CHỐT CHẶN ĐỔI ID: Bảo vệ nhãn khi mạng tải xong
    ...
    // QUÉT VÀ KHÓA HÀM LIÊN TỤC TRONG 2 GIÂY ĐẦU (Đề phòng các patch khác tải chậm)
    ...
    const intervalHook = setInterval(() => {
        applyAllHooks();
        hookAttempts++;
        if (hookAttempts > 40) clearInterval(intervalHook);
    }, 50);

    console.log("✅ PATCH 35 (V5): Đã fix tốc độ! Nhãn NEW sẵn sàng bắt khách ngay từ giây số 0!");
})();