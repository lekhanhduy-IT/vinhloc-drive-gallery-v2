        ...
        (function(){
            ...
            // Gắn patch cho nút Chọn Ảnh Xem Trước trong Popup Info
        ...
        window.uiPromptFolder = function(targetParentId = null, e = null) {
            ...
            btn.onclick = () => { 
                ...
                if (val) {
                    ...
                    const fakeId = 'temp_folder_' + Date.now();
                    const newItem = { id: fakeId, name: val, type: 'folder' };
                    
                    if (parentIdToUse === currentFolderId) {
                        if(folderStack.length === 1) {
                            appMeta[fakeId] = { type: currentCategory, desc: '', cover: '' };
                            saveLocalMeta();
                        }
                    ...
                    bgApiCall('createFolder', { name: val, folderId: parentIdToUse }).then(res => { 
                        if (res && res.success) {
                            const realId = res.folderId || res.id;
                            if (parentIdToUse === currentFolderId) {
                                const idx = currentDriveItems.findIndex(i => i.id === fakeId);
                                if(idx > -1) currentDriveItems[idx].id = realId;
                                
                                if(folderStack.length === 1) {
                                    appMeta[realId] = appMeta[fakeId];
                                    delete appMeta[fakeId];
                                    saveLocalMeta();
                                    bgApiCall('updateSingleMeta', { meta: { id: realId, name: val, type: currentCategory, desc: '', cover: '' } });
                                }
                                localStorage.setItem(`folder_${currentFolderId}`, JSON.stringify(currentDriveItems));
                            } else if (subFolderCache[parentIdToUse]) {
                                const idx = subFolderCache[parentIdToUse].findIndex(i => i.id === fakeId);
                                if(idx > -1) subFolderCache[parentIdToUse][idx].id = realId;
                            }
                        }
                    });
                } 
        ...
        // XỬ LÝ SỰ KIỆN CHUYỂN SUB-TAB ĐỔI TÊN
        ...
        // SỬA SỰ KIỆN CHUYỂN TAB ĐỂ RENDER LIST KHI BẤM VÀO TAB "ĐỔI TÊN"
            ...
            item.addEventListener('click', () => { 
                ...
                if(item.classList.contains('active')){ 
                    ...
                    if(item.dataset.panel==='panel-rename') {
                        // Gọi render danh sách nếu tab Tên riêng đang mở
        ...
        document.getElementById('btn-save-text-wm').addEventListener('click', () => {
            ...
            state.savedWatermarks.push(c.toDataURL()); saveStorageWMs(); renderWMLibrary(); showToast('<i class="fas fa-check mr-2"></i> Đã lưu thành Watermark');
        });
