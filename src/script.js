...
// =====================================================================
// 1. CỐ ĐỊNH THANH TIÊU ĐỀ MEGA-ROW KHI CUỘN (STICKY HEADER)
// =====================================================================
    ...
    if (folderStack.length === 1 && !isSearchMode) {
        ...
        if(folders.length > 0) {
            ...
            folderListEl.innerHTML = folders.map(item => {
                ...
                `;
                return `
                <div class="subfolder-row group bg-white border-b border-gray-200" onclick="loadFolder('${item.id}', '${item.name}', true)">
                    ${imgHtml}
                    <div class="flex-1 overflow-hidden">
                        <h4 class="text-sm font-bold text-gray-800 truncate item-name-${item.id}">${item.name}</h4>
                        <p class="text-[11px] text-gray-500 truncate mt-0.5 item-desc-${item.id} ${meta.desc ? '' : 'hidden'}">${meta.desc || 'Chưa có mô tả'}</p>
...
// =====================================================================
// 3. TÌM KIẾM THÔNG MINH (Fuzzy Search & Global Cache Search)
// =====================================================================
    ...
    window.smartSearchTimeout = setTimeout(async () => {
        ...
        try {
            ...
            if(res && res.success && res.data) {
                ...
                resultsArray = Array.from(localResults.values()); 
            }
        } catch(err) {
...
// =====================================================================
// PATCH MỚI: TỐI ƯU HÓA UPLOAD VÀ TẢI DỮ LIỆU ĐA TIẾN TRÌNH
// =====================================================================

// 1. ĐÓNG GÓI UI GIAO DIỆN TỪNG FILE ĐỂ TÁI SỬ DỤNG
window.renderSingleFile = function(item) {
    let isImage = item.mimeType.includes('image');
    
    // Ưu tiên dùng tempUrl (ảnh gốc dưới máy) kể cả khi đã up xong để chống nháy ảnh
    let imgUrl = item.tempUrl ? item.tempUrl : `https://drive.google.com/thumbnail?id=${item.id}&sz=w400`;
    let fullImgUrl = item.tempUrl ? item.tempUrl : `https://drive.google.com/thumbnail?id=${item.id}&sz=w2000`;
    ...
    // Vòng loading chỉ hiện khi isUploading = true
    let isTemp = item.isUploading ? `<div class="absolute inset-0 bg-white/70 flex flex-col items-center justify-center backdrop-blur-[2px] z-10 rounded-2xl"><div class="loader mb-2 border-blue-600"></div><span class="text-[10px] font-bold text-blue-600">Đang lưu...</span></div>` : '';
    
    return `
    <div id="file-wrap-${item.id}" class="fade-in-item bg-white p-2.5 rounded-2xl shadow-sm border border-gray-100 flex flex-col relative hover:shadow-md transition">
        ${isTemp}
        <div class="absolute top-2 right-2 z-20">
            <button onclick="window.toggleItemMenu('${item.id}', event)" class="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-blue-600 bg-white/90 backdrop-blur-md rounded-full shadow-sm"><i class="fas fa-ellipsis-v"></i></button>
            <div id="menu-${item.id}" class="hidden absolute right-0 mt-1 w-40 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 py-1 text-sm item-action-menu overflow-hidden">
...
// 2. SỬA LỖI MẤT ẢNH TRONG QUÁ TRÌNH LƯU DRIVE (KHÔNG XÓA TEMP_URL)
window.handleMultipleFileUpload = async function(event) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // isUploading = true để hiện vòng xoay xanh
        let newItem = { id: fakeId, name: file.name, mimeType: file.type, type: 'file', tempUrl: tempUrl, isUploading: true };
        tempFilesToUpload.push({ file: file, item: newItem });
    ...
    folderDataCache[currentFolderId] = currentDriveItems;
    // Bật render ngay lập tức để hiện mẻ ảnh vừa chọn
    renderItems(currentDriveItems, false, true);
    ...
    for (let obj of tempFilesToUpload) {
        ...
        reader.onload = async function(e) {
            ...
            if (res && res.success) {
                let idx = currentDriveItems.findIndex(i => i.id === obj.item.id);
                if(idx > -1) {
                    const realId = res.fileId || res.id;
                    const oldFakeId = currentDriveItems[idx].id;
                    
                    // QUAN TRỌNG: Gán ID thật từ Drive, Tắt vòng quay, NHƯNG GIỮ NGUYÊN tempUrl
                    currentDriveItems[idx].id = realId;
                    currentDriveItems[idx].isUploading = false;
                    folderDataCache[currentFolderId] = currentDriveItems; 
                    
                    // Render lại chỉ đúng ô ảnh đó để gỡ vòng quay (không render lại cả trang)
                    const oldNode = document.getElementById(`file-wrap-${oldFakeId}`);
                    if (oldNode) {
                        oldNode.outerHTML = window.renderSingleFile(currentDriveItems[idx]);
                    }
                }
            }
        };
...
// 3. TẢI VÀ HIỂN THỊ DỮ LIỆU CHIA NHỎ (PROGRESSIVE RENDERING)
window.renderItems = function(items, isSearchMode = false, isInstant = false) {
    const folderListEl = document.getElementById('folderList');
    const fileListEl = document.getElementById('fileList');
    
    // Dừng tiến trình nạp ảnh cũ nếu người dùng chuyển folder
    if (window.progressiveRenderTimer) {
        clearTimeout(window.progressiveRenderTimer);
        window.progressiveRenderTimer = null;
    }
    ...
    if (folderStack.length === 1 && !isSearchMode) {
        ...
        // BƯỚC 1: Render các thư mục con ngay lập tức
        if(folders.length > 0) {
            folderListEl.innerHTML = folders.map(item => {
                const meta = getMeta(item.id);
                const imgHtml = `
        ...
        // BƯỚC 2: Render các file (Ảnh) theo từng cụm (Chunking) để không lag
        if(files.length > 0) {
            // Nếu có ít ảnh hoặc đang có tác vụ upload (isInstant), nạp tất cả
            if (isInstant || files.length <= 10) {
                fileListEl.innerHTML = files.map(window.renderSingleFile).join('');
            } else {
                let idx = 0;
                const chunkSize = 12; // Render 12 ảnh mỗi lần nhịp
                
                // Render nhịp đầu tiên ngay lập tức để người dùng xem luôn
                const firstChunk = files.slice(0, chunkSize);
                fileListEl.innerHTML = firstChunk.map(window.renderSingleFile).join('');
                idx += chunkSize;
                
                // Hàm đệ quy render nối tiếp
                function nextChunk() {
                    if (idx >= files.length) return;
                    const chunk = files.slice(idx, idx + chunkSize);
                    // Dùng insertAdjacentHTML để nhét thêm vào cuối DOM (tránh giật)
                    fileListEl.insertAdjacentHTML('beforeend', chunk.map(window.renderSingleFile).join(''));
                    idx += chunkSize;
                    // Chờ 50ms (không làm đứng trình duyệt) rồi render tiếp 12 ảnh nữa
                    window.progressiveRenderTimer = setTimeout(nextChunk, 50);
                }
                
                // Bắt đầu nhịp nối tiếp
                window.progressiveRenderTimer = setTimeout(nextChunk, 50);
            }
        }
    }
